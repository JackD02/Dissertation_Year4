...
data = pd.read_csv(r"C:\UNI\Y4\Dissertation\D2 Deliverable\Data for Diss\4 Rangefinder Bed Experiment\RBT_Combined.csv")
external_set = pd.read_csv(r"C:\UNI\Y4\Dissertation\D2 Deliverable\Data for Diss\4 Rangefinder Bed Experiment\RBT4.csv", encoding='ISO-8859-1')

encoder = LabelEncoder()
encoder.fit(data[" Label"])

#timestamp values have no value in the ML aspect of this
X = data[[" rf Distance left", " rf Distance right"]] #keep only sensor values
...
print("\nImportances: ")
for i, feature in enumerate([" rf Distance left", " rf Distance right"]):
    print(f"{feature}: {importances[i]:.4f}")

### EXTERNAL SET
# Preprocess external data (apply the same preprocessing as the training data)
external_X = external_set[[" rf Distance left", " rf Distance right"]]

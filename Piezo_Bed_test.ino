...
void setup() {
  ...
  if(file){
    file.println("Timestamp (ms), Piezo Value Left, Piezo Value Right, Label");
    file.close();
  } else{ 
...
String classifyState(int piezo_left, int piezo_right){
  if(piezo_left > piezo_right && piezo_right != 0) return "left";
  else if (piezo_left < piezo_right && piezo_left != 0) return "right";
  else if (piezo_left == piezo_right) return "middle";
...
void loop() {
  ...
  if (millis() - startTime < duration){
    ...
    String label = classifyState(sensorValue1, sensorValue2);
